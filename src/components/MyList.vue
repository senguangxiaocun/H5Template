...
<template>
  <div p-layout-padding>
    ...
    <div
      ...
      items-center
      justify-between
      class="list-box rounded-[20px] bg-[#282329cc] asdsasdad"
    >
      <ul>
      ...
      <!-- <van-icon color="#fff" :name="rightIcon" @click="onClick(item.userId, index)" /> -->
      <img
        v-if="props.type !== 'follow'"
        :src="Followicon"
        width="24px"
        @click="onClick(item.userId, index)"
      />
      <!-- <img
        v-if="props.type === 'fans'"
        :src="Fansicon"
        width="24px"
        @click="onClick(item.userId, index)"
      /> -->
      <img
        v-if="props.type === 'blackList'"
        :src="Blackicon"
        width="24px"
        @click="onClick(item.userId, index)"
      />
